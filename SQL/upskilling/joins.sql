...
FROM	happiness_scores hs
		INNER JOIN country_stats cs   -- returns records existing in both tables
        ON hs.country = cs.country;
        
SELECT	hs.year, hs.country, hs.happiness_score,
		cs.country, cs.continent
FROM	happiness_scores hs
		LEFT JOIN country_stats cs   -- returns all from left and similar of right --most common
        ON hs.country = cs.country
...
SELECT	hs.year, hs.country, hs.happiness_score,
		cs.country, cs.continent
FROM	happiness_scores hs
		RIGHT JOIN country_stats cs  		 -- returns all from right and similar of left  -- not available for all dbs
        ON hs.country = cs.country
...
FROM	happiness_scores hs   -- left most table to serve as the query base - grounds query
		LEFT JOIN country_stats cs   -- all from happiness and common from country
			ON hs.country = cs.country   -- all values present in happiness table and null values present in country table (if value not present)
		LEFT JOIN inflation_rates ir   -- second left join 
			ON hs.year = ir.year AND hs.country = ir.country_name;  -- has all values from happiness table,
